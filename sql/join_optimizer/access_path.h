...
struct AccessPath {
  ...
  AccessPath()
      : count_examined_rows(false), pinned(false)
#ifndef NDEBUG
        ,
  ...
  bool count_examined_rows : 1;
    ...
    /// @brief is this AccessPath pinned by hash pinning or hints
  bool pinned : 1;
#ifndef NDEBUG
...
static_assert(sizeof(AccessPath) <= 145, //TODO Lisa find exact
...
inline void CopyBasicProperties(const AccessPath &from, AccessPath *to) {
  ...
  to->pinned = current_thd->pin ? from.pinned : false;
}
...
inline AccessPath *NewLimitOffsetAccessPath(THD *thd, AccessPath *child,
  ...
  path->pinned = thd->pin ? child->pinned : false; //redundant?
...
inline AccessPath *NewStreamingAccessPath(THD *thd, AccessPath *child,
  ...
  path->pinned = thd->pin ? child->pinned : false;
  return path;
}
